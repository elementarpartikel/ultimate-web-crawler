...
# -*- coding: utf-8 -*-
"""
Webbdammsugare Pro (v1.1) - Professionellt verktyg för webbskrapning
Skapad av Fredrik Eriksson

Funktioner: AI-optimerad, Hybrid, SQLite, Positiva/Negativa URL-filter
"""
...
class WebCrawler:
    def __init__(self, config: dict, msg_queue: queue.Queue):
        ...
        self.req_session.headers.update({'User-Agent': 'Mozilla/5.0 WebbdammsugarePro/1.1'})
        
        self.robot_parser = None
        if config["respect_robots"]: self._load_robots_txt()
        
        self.driver = None
        self._log(f"🚀 Initierar crawl för {self.domain} (V1.1 - Positiva filter)")
    ...
    def is_valid_url(self, url: str) -> bool:
        ...
        try:
            ...
            # NEGATIVT FILTER: Uteslut ord
            for kw in self.config.get("exclude_keywords", []):
                if kw and kw in lower_url: return False
                
            # POSITIVT FILTER: Kräv ord (Här löser vi Google Sites-problemet)
...
class AppGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Webbdammsugare Pro (v1.1)")
