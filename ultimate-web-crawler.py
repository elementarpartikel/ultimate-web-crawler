...
# -*- coding: utf-8 -*-
"""
Webbdammsugare Pro (v1.2) - Professionellt verktyg för webbskrapning
Skapad av Fredrik Eriksson

Funktioner: AI-optimerad, Hybrid, SQLite, Retries, Depth Limit, URL-normalisering
"""
...
def normalize_url(url: str, ignore_query_params: Optional[List[str]] = None) -> str:
    ...
    try:
        ...
        # FIX: Ta bort index.html och index.php för att undvika dubbletter
...
class CrawlDatabase:
    def __init__(self, db_path: str):
        ...
        # FIX: Återanvänd samma anslutning för mycket bättre prestanda
...
class PriorityURLQueue:
    ...
    def add_url(self, url: str, depth: int = 0, priority: int = CrawlPriority.MEDIUM.value):
        ...
        with self._lock:
            if normalized not in self.seen_urls:
                # FIX: Inkludera time.time() för att förhindra starvation och garantera FIFO vid samma prioritet
    ...
    def get_next(self) -> Optional[Tuple[int, str]]:
        try: 
            ...
            return (item[2], item[3]) # Returnerar (depth, url)
        except queue.Empty: 
            return None
...
class WebCrawler:
    def __init__(self, config: dict, msg_queue: queue.Queue):
        ...
        self.max_depth = config.get("max_depth", 0) # Nytt: Max djup
        ...
        # FIX: Konfigurera requests med Retry för stabilare HTTP-anrop
        ...
        self.driver = None
        self._log(f"🚀 Initierar crawl för {self.domain} (V1.2 - Pro Edition)")

    def _create_robust_session(self):
        """Skapar en stabil request-session som försöker igen vid tillfälliga serverfel."""
        session = requests.Session()
        session.headers.update({'User-Agent': 'Mozilla/5.0 WebbdammsugarePro/1.2'})
    ...
    def process_page(self, url: str, depth: int) -> bool:
        ...
        try:
            ...
            if canonical:
                ...
                # Vi lägger till canonical i kön och låter loopen hantera den, avbryter denna ifall den är exakt duplicerad
                if canonical != url:
            ...
            # Extrahera länkar (bara om vi inte nått maxdjup)
            if self.max_depth == 0 or depth < self.max_depth:
                ...
                for a in soup.find_all('a', href=True):
                    ...
                    if ext in self.doc_extensions and self.config["download_docs"]:
                        ...
                        # Queue next link with increased depth
...
class AppGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Webbdammsugare Pro (v1.2)")
...
* Körläge:
  ...
  - login_then_headless: Perfekt för intranät! Öppnar en synlig webbläsare så du kan logga in (du har 60 sekunder på dig). Därefter stängs fönstret och programmet dammsuger intranätet osynligt.
* Mapp: Välj var på din dator du vill att alla filer (texter, dokument, loggar) ska sparas.
...
"""
    ...
    def _build_ui(self):
        ...
        ttk.Label(arow3, text="Kräv ord i URL:").pack(side=tk.LEFT, padx=(10,0))
