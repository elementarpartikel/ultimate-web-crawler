...
# -*- coding: utf-8 -*-
"""
Webbdammsugare Pro (v2.0) - UI Refresh, uvloop, lxml & AI Edition
...
class WebCrawler:
    def __init__(self, config: dict, msg_queue: queue.Queue):
        ...
        self.driver = None
        self._log(f"🚀 Initierar crawl för {self.domain} (v2.0)")
    ...
    def extract_text(self, html: str) -> Tuple[str, str]:
        ...
        # FIX: Använder korrekta radbrytningar \n
            ...
            if self.config["incremental"] and cached and cached.get('hash') == content_hash:
                ...
                if len(page_text) > 50:
                    ...
                    with open(file_path, 'w', encoding='utf-8') as f:
                        # FIX: Rättade till spar-texten så filerna får vanliga radbrytningar
...
# ─────────────────────────────────────────────────────────────
#  ASYNC WEBB CRAWLER CORE (EXPERIMENTAL)
# ─────────────────────────────────────────────────────────────
class AsyncWebCrawler(WebCrawler):
    def __init__(self, config: dict, msg_queue: queue.Queue):
        ...
        self._log(f"🚀 Initierar ASYNC crawl för {self.domain} (v2.0)")
        ...
        try:
            ...
            if self.config["incremental"] and cached and cached.get('hash') == content_hash:
                ...
                if len(page_text) > 50:
                    ...
                    file_path = os.path.join(texts_dir, self.url_to_filename(url))
                    with open(file_path, 'w', encoding='utf-8') as f:
                        # FIX: Samma sak här, korrekta radbrytningar \n
                        f.write(f"KÄLLA: {url}\nTITEL: {title}\nHÄMTAD: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n{'='*60}\n\n{page_text}")
        ...
        try:
            ...
            async with asyncio.TaskGroup() as tg:
                while self.state != CrawlerState.STOPPED:
                    ...
                    tg.create_task(self.process_page(queue_item[1], queue_item[0]))
...
class AppGUI:
    def __init__(self, root: ctk.CTk):
        ...
        self.lang = "sv"
        self.texts = {
            "window_title": {"sv": "Webbdammsugare Pro (v2.0)", "en": "Web Crawler Pro (v2.0)"},
        ...
        try:
            for _ in range(20):
                ...
                if msg_type == "log":
                    ...
                    # FIX: Rättad radbrytning i loggen här (\n istället för \\n)
                    ...
                    visited, _, _, _, _ = data
                    if self.progress_bar and self.max_pages_entry.get() != "0":
                        self.progress_bar.set(visited / int(self.max_pages_entry.get()))
