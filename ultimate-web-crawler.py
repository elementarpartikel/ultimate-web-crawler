...
# -*- coding: utf-8 -*-
"""
Webbdammsugare Pro v1.0 (AI & RAG Edition) - Professionellt verktyg för webbskrapning
Skapad av Fredrik Eriksson

Funktioner: AI/RAG-optimerad, Hybrid, SQLite, Markdown-stöd (.md), Säker nerladdning, Inbyggd Hjälp
"""
...
class WebCrawler:
    def __init__(self, config: dict, msg_queue: queue.Queue):
        ...
        self.req_session.headers.update({'User-Agent': 'Mozilla/5.0 WebbdammsugarePro/1.0'})
        
        self.robot_parser = None
        if config["respect_robots"]: self._load_robots_txt()
        
        self.driver = None
        self._log(f"🚀 Initierar crawl för {self.domain} (Format: {self.save_format})")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for url_node in soup.find_all('url'):
                    ...
                    if loc and self.url_queue.add_url(loc.text.strip(), CrawlPriority.SITEMAP.value):
                        count += 1
...
class AppGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Webbdammsugare Pro (v1.0)")
...
* Uteslut ord i URL: Här kan du skriva in ord (separerade med kommatecken) för att undvika att ladda ner skräp. Skriver du in t.ex. "kalender, profilsida" kommer programmet att hoppa över alla webbadresser som innehåller dessa ord.
"""
    ...
    def _build_ui(self):
        ...
        # --- Top Frame för URL och Hjälp-knapp ---
        ...
        ttk.Combobox(row1, textvariable=self.format_var, values=[".md (Bäst för AI)", ".txt (Klassisk)"], state="readonly", width=18).pack(side=tk.LEFT, padx=(2, 15))
