...
# -*- coding: utf-8 -*-
"""
Webbdammsugare Pro (v5.0 AI & GDPR)
Skapad av Fredrik Eriksson

Funktioner: Playwright, Semantic Chunking, PII-tvätt, HEAD-requests, Grid-Layout, "Endast Dokument"-läge
"""
...
# ─────────────────────────────────────────────────────────────
#  HJÄLPFUNKTIONER & CHUNKING (UPPDATERAD FÖR RAG)
# ─────────────────────────────────────────────────────────────
...
def chunk_text(text: str, max_words=400, overlap_words=50) -> List[str]:
...
class AsyncWebCrawler:
    def __init__(self, config: dict, msg_queue: Optional[queue.Queue] = None):
        ...
        self.playwright_semaphore = asyncio.Semaphore(5)
        ...
        self._log(f"🚀 Initierar ASYNC crawl för {self.domain} (v5.0 AI & GDPR)")
            ...
            try:
                if method == 'HEAD':
                    ...
                    async with self.req_session.get(url, timeout=15) as resp:
                        ...
                        # FIX 2: Tydligt tillägg för html för att garantera att alla webbsidor godkänns
    ...
    async def _parse_sitemap(self, url: str):
        ...
        try:
            ...
            for url_node in soup.find_all('url'):
                ...
                if loc and self.url_queue.add_url(loc.text.strip(), depth=0, base_priority=CrawlPriority.SITEMAP.value): count += 1
    ...
    def extract_structured_data(self, html: str, url: str) -> Dict:
        ...
        if self.use_trafilatura and HAS_TRAFILATURA:
            try:
                ...
                if extracted:
                    full_text = self.clean_pii(extracted)
            except Exception as e:
        ...
        for el in soup.find_all(['h1', 'h2', 'h3', 'h4', 'p', 'ul', 'ol', 'table']):
            if el.name.startswith('h'):
                ...
                for row in rows:
                    ...
                    if cols: current_text.append(" | ".join(cols))
            else:
    ...
    async def process_page(self, url: str, depth: int) -> bool:
        ...
        try:
            ...
            data = self.extract_structured_data(html, url)
        ...
        try:
            ...
            with open(os.path.join(self.output_dir, "index.csv"), 'w', newline='', encoding='utf-8') as f:
                ...
                for url, title, date, content_hash in records: 
                    # FIX 1: Max 50 tecken för att matcha de faktiska filerna
                    safe_name = f"{slugify(urlparse(url).path)[:50]}_{content_hash[:6]}"
        ...
        try:
            ...
            async with asyncio.TaskGroup() as tg:
                while self.state != CrawlerState.STOPPED:
                    ...
                    if not queue_item:
                        # FIX 3: Läser av active_tasks tryggt inuti låset för att undvika race conditions
                        async with self.async_stats_lock:
                            tasks_are_zero = (active_tasks == 0)
...
class AppGUI:
    def __init__(self, root: ctk.CTk):
        ...
        self.lang = "sv"
        self.texts = {
            "window_title": {"sv": "Webbdammsugare Pro (v5.0 AI & GDPR)", "en": "Web Crawler Pro (v5.0 AI & GDPR)"},
    ...
    def _build_ui(self):
        ...
        self.exclude_entry = ctk.CTkEntry(tab_adv)
        ...
        self.require_entry = ctk.CTkEntry(tab_adv)
